(function () {
	...
	function getSelectedTable() {
		var selected = document.querySelector('input[name="database_scan_posts_table"]:checked');
	...
	function selectTable(tableName) {
		...
		inputs = document.querySelectorAll('input[name="database_scan_posts_table"]');
	...
	function request(action, tableName) {
		...
		body.append('database_scan_posts_table', tableName || '');
	...
	function getSortValue(finding, key) {
		...
		if (key === 'post_id' || key === 'user_id' || key === 'size') {
			return parseInt(finding[key], 10) || 0;
		}
	...
	function defaultSectionState(sectionKey) {
		if (sectionKey === 'large_post_content') {
			return {
				page: 1,
				sortKey: 'size',
				sortDirection: 'desc'
	...
	function renderUserIdCell(finding) {
		...
		if (!displayName) {
			appendText(cell, text(userId));
			return cell;
		}

		var span = createElement('span', 'cws-posts-scan-user-id', text(userId));
		span.title = displayName;
		span.setAttribute('aria-label', format(strings.userIdLabel || 'User ID %1$s (%2$s)', text(userId), displayName));
	...
	function renderFindingRow(tbody, finding, sectionKey) {
		...
		row.appendChild(renderUserIdCell(finding));
		row.appendChild(createElement('td', '', finding.post_title || ''));
		row.appendChild(createElement('td', '', finding.post_type || ''));
		row.appendChild(createElement('td', '', finding.post_status || ''));
		row.appendChild(createElement('td', '', sizeFormat(finding.size || 0)));
		row.appendChild(createElement('td', '', finding.detail || ''));

		excerptCell = createElement('td', sectionKey === 'large_post_content' ? 'cws-posts-scan-excerpt' : '', finding.excerpt || '');
	...
	function renderSection(resultsEl, sectionKey, section) {
		...
		var sectionEl = createElement('div', 'cws-report-section cws-posts-scan-section' + (sectionKey === 'large_post_content' ? ' cws-posts-scan-section-full-width' : ''));
		...
		headerRow.appendChild(sortableHeader(strings.postId, sectionKey, 'post_id'));
		headerRow.appendChild(sortableHeader(strings.userId, sectionKey, 'user_id'));
		headerRow.appendChild(sortableHeader(strings.title, sectionKey, 'post_title'));
		headerRow.appendChild(sortableHeader(strings.type, sectionKey, 'post_type'));
		headerRow.appendChild(sortableHeader(strings.status, sectionKey, 'post_status'));
	...
	function renderSummary(resultsEl, result) {
		...
		if (result.posts_table) {
			panel.appendChild(createElement('p', '', format(strings.scannedTable, result.posts_table)));
		}
	...
	function renderBottomActions(resultsEl) {
		var actions = createElement('div', 'cws-posts-scan-report-actions');
	...
	function renderResult(result) {
		...
		if (result.posts_table) {
			selectTable(result.posts_table);
		}
	...
	function handleScan() {
		...
		request('choctaw_wp_security_posts_scan', getSelectedTable()).then(function (data) {
			sectionState = {};
			showNotice('', 'success');
	...
	function handleBaselineReset() {
		...
		request('choctaw_wp_security_posts_scan_baseline_reset', getSelectedTable()).then(function (data) {
			showNotice(data.message || '', 'success');
			if (data.posts_table) {
				selectTable(data.posts_table);
			}
		}).catch(function (error) {
	...
	function init() {
		...
		form.addEventListener('submit', function (event) {
			...
			var isBaselineReset = submitter && submitter.name === 'choctaw_wp_security_posts_scan_baseline_reset';
