...
- Risk classification and finding explanations
- Accepted / dismissed / needs-review status storage
...
2. User reviews why/how/evidence in Desktop UI (data from JSON).
3. Status updates via `findings set-status`; plugin persists.
4. Desktop may keep its own history copy but treats plugin status as source of truth for the site.
...
- How Desktop stores and syncs historical reports.
- Multi-site bulk scan concurrency limits.
- Auth model when SSH user maps to a specific WP user (`wp --user=`).
