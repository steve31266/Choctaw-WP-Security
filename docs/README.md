...
**Findings System:** Phase 1, 2, **3.0**, **3.1**, **3.2**, **3.3**, **3.4**, and **3.4.5** (object-level Findings + categories; persistence, Uploads + MU-Plugins + Verify Checksums + Exposed Files + Database options + WP-Cron, Network Admin shell, related-findings UI, structured guidance) are implemented — see [CoreGuard Findings System.md](CoreGuard%20Findings%20System.md) §11 and §18. Remaining scanner migrations are Phase **3.5–3.7**; prototype store wind-down is **3.8**. CLI/JSON/Desktop remain Phase 4/5.
