	...
	private function build_report_from_findings( Sassh_Findings_Service $service, $execution_id, array $run_meta ) {
		...
		foreach ( $rows as $row ) {
			...
			foreach ( $categories as $cat ) {
				$rid = isset( $cat['rule_id'] ) ? (string) $cat['rule_id'] : '';
				if ( '' !== $rid ) {
					$rules[] = str_replace( '-', '_', $rid );
				}
				$labels[] = isset( $cat['category_label'] ) ? (string) $cat['category_label'] : ( isset( $cat['title'] ) ? (string) $cat['title'] : $rid );
			}
			...
			$findings[] = array(
				...
				'previously_detected'     => ( isset( $row['previously_detected'] ) && is_array( $row['previously_detected'] ) ) ? $row['previously_detected'] : array(),
				'extra_rule_count'        => isset( $row['extra_rule_count'] ) ? (int) $row['extra_rule_count'] : max( 0, count( $categories ) - 1 ),
				'primary_rule_id'         => isset( $row['primary_rule_id'] ) ? $row['primary_rule_id'] : '',
				'rule_id'                 => isset( $row['primary_rule_id'] ) ? $row['primary_rule_id'] : '',
				'rules'                   => $rules,
