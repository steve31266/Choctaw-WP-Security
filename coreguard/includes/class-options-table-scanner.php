...
class Choctaw_Wp_Security_Options_Table_Scanner {
	...
	private function scan_large_autoload( array &$findings ) {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT option_id, option_name, LENGTH(option_value) AS option_size,
					LEFT(option_value, %d) AS option_excerpt,
					LEFT(option_value, %d) AS option_full
				FROM {$table}
				WHERE autoload = %s
				ORDER BY option_size DESC",
				Choctaw_Wp_Security_Options_Scan_Patterns::LARGE_AUTOLOAD_PREVIEW_LENGTH,
				Choctaw_Wp_Security_Options_Scan_Patterns::FULL_VALUE_MAX_LENGTH,
				'yes'
			),
			ARRAY_A
		);
		...
		foreach ( $rows as $row ) {
			...
			$preview  = isset( $row['option_excerpt'] ) ? (string) $row['option_excerpt'] : '';
			$full     = isset( $row['option_full'] ) ? (string) $row['option_full'] : $preview;
	...
	private function enrich_sections( array $sections ) {
		...
		foreach ( $sections as $section_key => &$section ) {
			...
			foreach ( $section['findings'] as $index => &$finding ) {
				...
				$risk     = Choctaw_Wp_Security_Options_Scan_Patterns::map_severity_to_risk( $severity, $section_key );
				$guidance = Choctaw_Wp_Security_Options_Scan_Patterns::resolve_detail_guidance( $section_key, $risk );
				...
				$fingerprint = Choctaw_Wp_Security_Finding_Status_Store::fingerprint_options_finding( $section_key, $finding );
				$finding['fingerprint'] = $fingerprint;
				$finding['id']          = $fingerprint;

				if ( ! isset( $finding['full_value'] ) ) {
					$finding['full_value'] = isset( $finding['excerpt'] ) ? (string) $finding['excerpt'] : '';
				}
	...
	private function build_summary( array $sections ) {
		$summary = array(
			...
			'safe'       => 0,
			'info'       => 0,
			'warning'    => 0, // Legacy key for older UI consumers.
		);
	 ...
	 * @param string      $full_value       Full option value for the detail panel.
	 * @return array<string, mixed>
	 */
	private function make_finding( $id, $severity, $option_name, $size, $detail, $excerpt = '', $option_id = null, $option_id_label = '', $full_value = '' ) {
		if ( null === $option_id ) {
			$option_id = $this->get_option_id_for_name( $option_name );
		}

		$prepared = $this->prepare_full_value( '' !== $full_value ? $full_value : $excerpt );
		...
		$finding = array(
			...
			'full_value'         => $prepared['contents'],
			'contents_truncated'  => ! empty( $prepared['truncated'] ),
		);
