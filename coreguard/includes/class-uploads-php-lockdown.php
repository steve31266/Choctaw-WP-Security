...
class Choctaw_Wp_Security_Uploads_Php_Lockdown {
	...
	const MARKER_BEGIN = '# BEGIN CoreGuard';
	const MARKER_END   = '# END CoreGuard';

	const LEGACY_MARKER_BEGIN = '# BEGIN Choctaw WP Security';
	const LEGACY_MARKER_END   = '# END Choctaw WP Security';
	...
	private function content_has_managed_block( $contents ) {
		$has_current = false !== strpos( $contents, self::MARKER_BEGIN )
			&& false !== strpos( $contents, self::MARKER_END );
		$has_legacy  = false !== strpos( $contents, self::LEGACY_MARKER_BEGIN )
			&& false !== strpos( $contents, self::LEGACY_MARKER_END );
	 ...
	 *
	 * Also strips any leftover legacy Choctaw WP Security markers so upgrades
	 * do not leave duplicate managed blocks.
	 *
	...
	private function replace_managed_block( $contents, $block ) {
		$updated = $this->strip_marker_block( $contents, self::MARKER_BEGIN, self::MARKER_END );
		$updated = $this->strip_marker_block( $updated, self::LEGACY_MARKER_BEGIN, self::LEGACY_MARKER_END );
		$updated = trim( $updated );
	...
	/**
	 * Remove one BEGIN/END marker pair from .htaccess content.
	 *
	 * @param string $contents File contents.
	 * @param string $begin    Begin marker line.
	 * @param string $end      End marker line.
	 * @return string
	 */
	private function strip_marker_block( $contents, $begin, $end ) {
		$pattern = '/\s*' . preg_quote( $begin, '/' ) . '.*?'
			. preg_quote( $end, '/' ) . '\s*/s';
