(function () {
	...
	var config = window.choctawWpSecurityUploadsFolder || {};
	...
	function showNotice(message, type) {
		var notices = document.getElementById('cws-uploads-folder-js-notices');
		if (!notices) {
			return;
		}
	...
	function setBusy(isBusy, message) {
		var form = document.getElementById('cws-uploads-folder-form');
		var resultsEl = document.getElementById('cws-uploads-folder-js-results');
		if (!form) {
			return;
		}
	...
	function filteredFindings(findings) {
		...
		return findings.filter(function (finding) {
			...
			if (search) {
				var haystack = text(finding.path).toLowerCase();
	...
	function renderDetailPanel(finding) {
		var grid = createElement('div', 'cws-report-detail-grid cws-uploads-folder-detail-grid');
		var left = createElement('div', 'cws-uploads-folder-detail-left');
		var right = createElement('div', 'cws-uploads-folder-detail-right');
		var infoPanel = createElement('div', 'cws-uploads-folder-info-panel');
		var infoList = document.createElement('dl');
		var contentsBlock = createElement('div', 'cws-uploads-folder-contents');
		var textarea = document.createElement('textarea');
		var whyBlock = createElement('div', 'cws-uploads-folder-why-block');
		var howBlock = createElement('div', 'cws-uploads-folder-how-block');

		infoPanel.appendChild(createElement('h4', '', strings.infoPanel || 'Info'));
		appendInfoField(infoList, strings.lastModified || 'Last Modified', finding.modified_label || '—');
		...
		if (window.CwsReportStatus) {
			window.CwsReportStatus.appendDismissControls(right, finding, config.scanType || 'uploads-folder', function () {
				uiState.expandedId = '';
				renderResult(resultState);
			});
		}
	...
	function renderTable(parent, findings) {
		var section = createElement('div', 'cws-report-section cws-uploads-folder-section');
		var sorted = sortedFindings(filteredFindings(findings));
		var pagination = paginate(sorted);
		var table = createElement('table', 'widefat striped cws-core-checksum-table cws-uploads-folder-table');
	...
	function renderResult(result) {
		var resultsEl = document.getElementById('cws-uploads-folder-js-results');
		var fallback = document.getElementById('cws-uploads-folder-fallback-results');
		if (!resultsEl || !result) {
			return;
		}
		resultState = result;
		...
		var button = document.querySelector('#cws-uploads-folder-form input[name="choctaw_wp_security_uploads_folder_scan"]');
	...
	function handleScan() {
		...
		request('choctaw_wp_security_uploads_folder_scan').then(function (data) {
			uiState.page = 1;
			uiState.expandedId = '';
			showNotice('', 'success');
